...
export interface RunResult {
  ...
  modelTag?: string;
  /**
   * True when this reply used a brand-new Claude session because `--resume` failed
   * (rate limits, gateway transcript errors, timeout recovery, etc.). Not set for the
   * very first message ever (`isNew` only).
   */
...
const activeClaudeProcesses = new Map<number, ActiveClaudeProcess>();

function trackActiveClaudeProcess(proc: ReturnType<typeof Bun.spawn>, name: string): void {
  const pid = proc.pid;
  if (!pid || pid <= 0) return;
  activeClaudeProcesses.set(pid, { pid, name, startedAt: Date.now(), proc });
}

function untrackActiveClaudeProcess(proc: ReturnType<typeof Bun.spawn>): void {
  const pid = proc.pid;
  if (!pid || pid <= 0) return;
...
export function interruptActiveRuns(reason: string = "manual interrupt"): {
  ...
  for (const entry of activeClaudeProcesses.values()) {
    const ageSec = Math.max(1, Math.floor((Date.now() - entry.startedAt) / 1000));
    details.push(`${entry.name} pid=${entry.pid} age=${ageSec}s`);
    try { entry.proc.kill("SIGTERM"); } catch {}
    setTimeout(() => { try { entry.proc.kill("SIGKILL"); } catch {} }, 5000);
  }
  if (details.length > 0) {
    console.warn(`[${new Date().toLocaleTimeString()}] Interrupted ${details.length} active Claude run(s): ${reason}`);
  }
  return { interrupted: details.length, details };
}
...
function isOpenAiCompatTranscriptError(stdout: string, stderr: string): boolean {
  ...
  return (
    ...
    /tool.use concurrency/i.test(t) ||
    /Invalid messages at \d+/i.test(t)
  );
}
...
async function runClaudeOnce(
  ...
  trackActiveClaudeProcess(proc, "runClaudeOnce");
...
async function execClaude(
  ...
  // Force-fallback mode: use fallback model as primary (for testing)
  if (isForceFallbackMode() && (fallbackConfig.model.trim() || fallbackConfig.api.trim())) {
  ...
  if (primaryRateLimit && hasModelConfig(fallbackConfig) && !sameModelConfig(primaryConfig, fallbackConfig)) {
    ...
    // No distinct fallback configured (or same model): start a fresh primary session
    if (exec.exitCode !== 0 && (!hasModelConfig(fallbackConfig) || sameModelConfig(primaryConfig, fallbackConfig))) {
      console.warn(
  ...
  if (!rateLimitMessage && exitCode === 0 && isPlaceholderAssistantOutput(stdout)) {
    stdout =
      "Il modello non ha restituito testo utile (solo un segnaposto dal provider). Ripeti la domanda o prova /useprimary se sei sul fallback.";
    console.warn(
      `[${new Date().toLocaleTimeString()}] Replaced placeholder assistant output for session ${sessionId.slice(0, 8)}…`,
    );
  }
  ...
  const result: RunResult = {
    ...
    exitCode,
    modelTag,
    freshSessionStart: usedFreshSessionStart,
  };
  ...
  if (COMPACT_TIMEOUT_ENABLED && exitCode === 124 && !isNew && existing) {
    ...
    if (compactOk) {
      ...
      // Retry still failed — fall through to try fallback below
      exec = { rawStdout: retryExec.rawStdout, stderr: retryExec.stderr, exitCode: retryExec.exitCode };
    }
    // Compact failed or compact+retry failed — try fresh fallback session
    if (!usedFallback && hasModelConfig(fallbackConfig) && !sameModelConfig(primaryConfig, fallbackConfig)) {
      console.warn(
  ...
  // Timeout recovery may set usedFreshSessionStart after result was built.
  result.freshSessionStart = usedFreshSessionStart;
...
async function streamClaude(
  ...
  trackActiveClaudeProcess(proc, "streamClaude");
